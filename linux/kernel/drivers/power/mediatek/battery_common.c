...
#if defined(CONFIG_MTK_BREATHLED_FY6203) //runyee zhou add breath led
extern int breathled_enable_ext(int mode);
#endif
...
#if 0 //runyee zhou del, modify low bat soc %1
...
#if defined(CONFIG_MTK_BREATHLED_FY6203) //runyee zhou add breath led
static void runyee_breathled_poweroff_check(void)
{
#if 0 //zhou del	
#ifdef CONFIG_MTK_KERNEL_POWER_OFF_CHARGING   
	if (g_platform_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT
       ...
       printk("runyee_breathled_poweroff_check BMT_status.SOC=%d \n",BMT_status.SOC);	  
		  if( (BMT_status.SOC>=0) &&(BMT_status.SOC<15) ){
		     breathled_enable_ext(1);
		  }else if( (BMT_status.SOC>=15) &&(BMT_status.SOC<=90)){
		     breathled_enable_ext(3);		
		  }else if( (BMT_status.SOC>90) &&(BMT_status.SOC<=100)){  		  
		     breathled_enable_ext(2);      
	    }   
	}
#endif
	#endif 
...
#if defined(CONFIG_MTK_BREATHLED_FY6203)
  runyee_breathled_poweroff_check();
#endif	
}
...
//	printk("[battery_common]__batt_parse_node, %s=%d\n",node_srting, val);
}
