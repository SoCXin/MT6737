...
struct i2c_client *power_client;//zhou add power_client
#if TOUCH_FILTER
...
printk("GT9xx tpd_down val, x=%d, y=%d.\n",x,y); //
...
if (tpd_dts_data.use_tpd_button) {
	...
	#else
	//runyee zhou modify tpd_down up key
	        key1 = (key_value & 0x01);
	        key2 = (key_value & 0x02);
	        key3 = (key_value & 0x04);
			...
			{
	            //tpd_down(key_1, 0, 0);
	             tpd_down(tpd_dts_data.tpd_key_dim_local[0].key_x, tpd_dts_data.tpd_key_dim_local[0].key_y, 0, 0);
	        }
	        ...
	        {
	            //tpd_down(key_2, 0, 0);
	            tpd_down(tpd_dts_data.tpd_key_dim_local[1].key_x, tpd_dts_data.tpd_key_dim_local[1].key_y, 0, 0);
			}
	        else if ((key2_old == 2) & (key2 == 0))
	        {
	            //tpd_up(key_2, 0);
	             tpd_up(tpd_dts_data.tpd_key_dim_local[1].key_x, tpd_dts_data.tpd_key_dim_local[1].key_y, 0);
	            }
	        if (key3 == 4)
		    {
	            //tpd_down(key_3, 0, 0);
	            tpd_down(tpd_dts_data.tpd_key_dim_local[2].key_x, tpd_dts_data.tpd_key_dim_local[2].key_y, 0, 0);
	        }
	        else if ((key3_old == 4) & (key3 == 0))
		    {
	            //tpd_up(key_3, 0);
	            tpd_up(tpd_dts_data.tpd_key_dim_local[2].key_x, tpd_dts_data.tpd_key_dim_local[2].key_y, 0);
		    }
	        key1_old = key1;
	        key2_old = key2;
	        key3_old = key3;
...
static int __init tpd_driver_init(void)
{
	...
	if (tpd_driver_add(&tpd_device_driver) < 0)
		printk("add gt9xx driver failed");
...
{
	printk("MediaTek gt91xx touch panel driver exit");
	printk("[Gt9xx]===%s,===========%d. \n", __func__, __LINE__);
