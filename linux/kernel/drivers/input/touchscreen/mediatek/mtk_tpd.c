...
//runyee zhou add device info,20161012
...
void tpd_get_dts_info(void)
{
	...
	if (node1) {
		...
		printk("[tpd_get_dts_info][tpd]use-tpd-button = %d, tpd-max-touch-num==%d\n", tpd_dts_data.use_tpd_button ,tpd_dts_data.touch_max_num);
		...
		if (tpd_dts_data.use_tpd_button) {
			...
			printk("[tpd_get_dts_info][tpd]tpd_dts_data.tpd_key_num==%d\n", tpd_dts_data.tpd_key_num);
			...
			for (i = 0; i < 4; i++) {
			  printk("[tpd_get_dts_info][tpd]key[%d].key = %d\n", i, tpd_dts_data.tpd_key_local[i]);
				printk("[tpd_get_dts_info][tpd]key[%d].key_x = %d\n", i, tpd_dts_data.tpd_key_dim_local[i].key_x);
			  printk("[tpd_get_dts_info][tpd]key[%d].key_y = %d\n", i, tpd_dts_data.tpd_key_dim_local[i].key_y);
				printk("[tpd_get_dts_info][tpd]key[%d].key_W = %d\n", i, tpd_dts_data.tpd_key_dim_local[i].key_width);
				printk("[tpd_get_dts_info][tpd]key[%d].key_H = %d\n", i, tpd_dts_data.tpd_key_dim_local[i].key_height);
			}
		}
