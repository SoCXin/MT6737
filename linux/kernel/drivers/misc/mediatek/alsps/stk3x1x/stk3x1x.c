...
/*----------------------------------------------------------------------------*/
#ifdef STK_ALS_FIR
	#define STK_FIR_LEN	3 //8 //zhou modify
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
    ...
    int32_t als_comperator_A;
    int32_t als_comperator_D;	
	u16 als_data;
	int32_t ir_data, tmp;
	u32 als_data_u32;
	...
	if(obj->ir_code)
	{
		obj->als_correct_factor = 1000; //CWF
			...
			obj->ir_code > STK_IRC_MIN_IR_CODE)
		{
			...
			als_comperator_D = als_data * 8;
			als_comperator_A = als_data * 39;
			
			if(obj->ir_code > als_comperator_A) 
				obj->als_correct_factor = 750;  // A
			else if(obj->ir_code > als_comperator_D)
				obj->als_correct_factor = 750;  // D
		}
...
#ifndef CALI_EVERY_TIME
	atomic_set(&obj->ps_high_thd_val, obj->ps_stat_data[1] + obj->stk_ht_n_ct*3); 
	atomic_set(&obj->ps_low_thd_val, obj->ps_stat_data[1] + obj->stk_lt_n_ct*3); 		
#else						
	obj->ps_high_thd_boot = obj->ps_stat_data[1] + obj->stk_ht_n_ct*3;
	obj->ps_low_thd_boot = obj->ps_stat_data[1] + obj->stk_lt_n_ct*3;
	atomic_set(&obj->ps_high_thd_val, obj->ps_high_thd_boot); 
	atomic_set(&obj->ps_low_thd_val, obj->ps_low_thd_boot); 
#endif
