...
RECALIBRATION:
	...
	for (i = 5; i < 15; i++) 
	{
		printk("%s: sun temp_pdata = %d\n", __func__, temp_pdata[i]);
		sum_of_pdata = sum_of_pdata + temp_pdata[i];
	}

	data->crosstalk_sun = sum_of_pdata/10;
	printk("%s: sum_of_pdata = %d  sun cross_talk = %d\n",
...
*/
	...
	{
			printk("zhou====pa122 disaple ps sensor\n");
...
int pa122_enable_als(struct i2c_client *client, int enable)
{
	...
	if(enable == 1)
	{
		printk("zhou====pa122 enable als sensor\n");
	...
	{	
		printk("zhou====pa122 disaple als sensor\n");
...
static int pa122_check_intr_als(struct i2c_client *client) 
{
	...
	printk("level:%d, alsht:%d, alslt:%d\n",level,
...
static int als_enable_nodata(int en)
{
	...
	printk("pa122_obj als enable value = %d\n", en);
...
static int als_get_data(int *value, int *status)
{
	...
	if (err) {
		...
		*value = pa122_get_als_value(obj, obj->als);
		//*value = obj->als * 110 / 100;  
	     *value = obj->als * 1100 / 100;   //runyee zhou ALS  sensitivity regulation
