...
static DEFINE_MUTEX(leds_pmic_mutex);
    ...
    int keyled_en_gpio;
    struct device_node	*keyled_of_node; 
#endif
...
int mt_led_set_pwm(int pwm_num, struct nled_setting *led)
{
    ...
    breathled_enable_ext(8); //all led OFF
#endif		
		break;
...
#if defined(CONFIG_MTK_BREATHLED_FY6203)		
    if(pwm_num==0)
       breathled_enable_ext(1); //red led ON
    else if(pwm_num==1)
       breathled_enable_ext(2); //green led ON   
    else if(pwm_num==2)
       breathled_enable_ext(3); //blue led ON                
#endif		
		break;
...
#if defined(CONFIG_MTK_BREATHLED_FY6203)		
    if(pwm_num==0)
       breathled_enable_ext(9); //red led breath
    else if(pwm_num==1)
       breathled_enable_ext(10); //green led breath   
    else if(pwm_num==2)
       breathled_enable_ext(11); //blue led breath                 
#endif						  
	}
...
int mt_mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	case MT65XX_LED_MODE_PMIC:
		...
		if ((strcmp(cust->name, "button-backlight") == 0)) {
		  ...
		  #if defined(CONFIG_RUNYEE_LEDS_BUTTON)
        if(level)
         	gpio_set_value(keyled_en_gpio,1);
        else
         	gpio_set_value(keyled_en_gpio,0); 	
         return 0;
      #else  
...
MODULE_AUTHOR("jst <cheng.zhou@runyee.com.cn>");
