...
/* add by jst for flashlight debug begin */
struct pinctrl_state *flash_en_h = NULL;
struct pinctrl_state *flash_en_l = NULL;
struct pinctrl_state *flash_mo_h = NULL;
struct pinctrl_state *flash_mo_l = NULL;
/* add by jst for flashlight debug end */
int mtkcam_gpio_init(struct platform_device *pdev)
{
	...
	/* add by jst for flashlight debug begin */
	flash_en_h = pinctrl_lookup_state(camctrl, "flash_en_1");
	if (IS_ERR(flash_en_h)) {
		ret = PTR_ERR(flash_en_h);
		pr_debug("%s : pinctrl err, flash_en_h\n", __func__);
	}
	flash_en_l = pinctrl_lookup_state(camctrl, "flash_en_0");
	if (IS_ERR(flash_en_l)) {
		ret = PTR_ERR(flash_en_l);
		pr_debug("%s : pinctrl err, flash_en_l\n", __func__);
	}

	flash_mo_h = pinctrl_lookup_state(camctrl, "flash_mo_1");
	if (IS_ERR(flash_mo_h)) {
		ret = PTR_ERR(flash_mo_h);
		pr_debug("%s : pinctrl err, flash_mo_h\n", __func__);
	}
	flash_mo_l = pinctrl_lookup_state(camctrl, "flash_mo_0");
	if (IS_ERR(flash_mo_l)) {
		ret = PTR_ERR(flash_mo_l);
		pr_debug("%s : pinctrl err, flash_mo_l\n", __func__);
	}	
...
if (On) {
    printk("[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
		...
		ISP_MCLK1_EN(1);
		printk("ISP_MCLK1_EN ok!!\n");
		
    if ((currSensorName && (0 == strcmp(currSensorName, "mn34152mipiraw"))) ||
		    (currSensorName && (0 == strcmp(currSensorName, "gc5024mipiraw" ))) ||
		    (currSensorName && (0 == strcmp(currSensorName, "ov5648mipi" ))) 
	  ){
