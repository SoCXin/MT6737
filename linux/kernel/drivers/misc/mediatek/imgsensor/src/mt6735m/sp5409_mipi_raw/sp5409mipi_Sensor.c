...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = SP5409MIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
{{ 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0, 0,2592,  1944}, // Preview 2112*1558
  { 2592, 1944, 0, 0, 2592, 1944, 2592, 1944, 0000, 0000, 2592, 1944, 0, 0,2592, 1944}, // capture 4206*3128
...
static void set_shutter(kal_uint16 shutter)
{
	...
	// Update Shutter
        ...
        write_cmos_sensor(0x04, shutter  & 0xFF); 
        write_cmos_sensor(0x01, 0x01); 
	printk("sp5409-Exit! shutter =%d, framelength =%d\n", shutter,imgsensor.frame_length);
...
write_cmos_sensor(0xfd,0x01);
write_cmos_sensor(0x06,0x00);//vblank 原为0x01改为0x00解决坏线问题150831
...
write_cmos_sensor(0x18,0x60);//gain为6x
write_cmos_sensor(0x19,0x40);//gain为4x
...
{
	printk("SP5409CaptureSetting_2lane enter! currefps:%d\n",currefps);
		...
		do {
            ...
            *sensor_id = ((read_cmos_sensor(0x02) << 8) | read_cmos_sensor(0x03));
			//*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
				printk("sp5409_get_imgsensor_id: 0x%x\n", *sensor_id);
				
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("sp5409_get_imgsensor_id, i2c read id: 0x%x == sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				return ERROR_NONE;
			}
