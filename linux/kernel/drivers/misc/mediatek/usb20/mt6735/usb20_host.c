...
#ifdef ID_PIN_USE_EX_EINT //change by jst for iot02 debug
...
#else
	#if 0 //del by jst for iot02 debug[NULL pointer panic]
	u32 phy_id_pull = 0;

	phy_id_pull = __raw_readl((unsigned long *)U2PHYDTM1);
	phy_id_pull |= ID_PULL_UP;
	__raw_writel(phy_id_pull, (unsigned long *)U2PHYDTM1);
