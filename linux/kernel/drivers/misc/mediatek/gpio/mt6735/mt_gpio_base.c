...
/* add by jst: change gpio driving current begin */
int mt_set_gpio_drv_current(unsigned long pin, unsigned long drv_param)
{
	unsigned long reg;
...
EXPORT_SYMBOL(mt_set_gpio_drv_current);
/* add by jst: change gpio driving current end */
